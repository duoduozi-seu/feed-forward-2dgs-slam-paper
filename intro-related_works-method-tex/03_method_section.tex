...
系统使用 DA3（基于 Depth Anything v3 的单目深度估计模型）为每个候选 surfel 提供初始法线方向。DA3 对当前帧 RGB 图像推理输出相对深度图后，系统通过最小二乘仿射对齐将其与稀疏 LiDAR 深度配准，得到与真实尺度一致的稠密深度图，再以中心差分计算逐像素法线。按候选点的像素坐标在法线图上采样，得到每个候选点的相机空间法线 $\mathbf{n}_{\text{cam}} \in \mathbb{R}^3$。
