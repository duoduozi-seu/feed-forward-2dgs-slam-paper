...
The feed-forward regression network is trained on R3LIVE sequences \datasetname{hku\_campus\_seq\_02}, \datasetname{hku\_campus\_seq\_03}, \datasetname{hkust\_campus\_seq\_02}, \datasetname{hkust\_campus\_seq\_03}, and our self-collected sequences \datasetname{seu\_campus\_seq\_02}, \datasetname{seu\_campus\_seq\_03}. Training uses the Adam optimizer with a batch size of 8192 (point-level) and gradient clipping. The learning rate is set to $5\times10^{-4}$ for a total of 150 epochs. Loss weights are $\lambda_{\text{scale}}{=}1.0$, $\lambda_{\text{rot}}{=}1.0$, $\lambda_{\text{dc}}{=}1.0$, $\lambda_{\text{rest}}{=}0.1$, $\lambda_{\text{opac}}{=}1.0$. The context window size is set to 15.
