...
\author{Puwei Zhao$^{\dagger}$, Yiduo Zhou$^{\dagger}$, Yiming Fan$^{\dagger}$, Kun Qian$^{*}$, and Tong Shi%
\thanks{$\dagger$ Puwei Zhao, Yiduo Zhou, and Yiming Fan contributed equally to this work.}%
\thanks{$*$ Corresponding author: Kun Qian.}
\thanks{Puwei Zhao is with the School of Automation, Southeast University, Nanjing, China (e-mail: 213233299@seu.edu.cn).}%
\thanks{Yiduo Zhou is with the School of Automation, Southeast University, Nanjing, China (e-mail: 213233816@seu.edu.cn).}%
\thanks{Yiming Fan is with the School of Automation, Southeast University, Nanjing, China (e-mail: 213233574@seu.edu.cn).}%
\thanks{Tong Shi is with the School of Automation, Southeast University, Nanjing, China (e-mail: shitong@seu.edu.com).}%
\thanks{Kun Qian is with the Southeast University Shenzhen Research Institute, Shenzhen, China, and also with the School of Automation, Southeast University, Nanjing, China (e-mail: kqian@seu.edu.cn).}%
...
We validate the effectiveness of the proposed method from two aspects. First, we conduct quantitative and qualitative comparisons with MonoGS~\cite{matsuki2024gaussian}, Co-SLAM~\cite{wang2023co}, MM3DGS-SLAM~\cite{sun2024mm3dgs}, and Gaussian-LIC2~\cite{lang2025gaussianlic2} on the public R3LIVE (Avia) dataset~\cite{lin2022r} and the MCD (OS1-64) dataset~\cite{nguyen2024mcd}. Second, we perform an ablation study of the feedforward regressor on the \texttt{hku\_campus\_00} sequence. For novel-view rendering quality, we use PSNR, SSIM, and LPIPS as evaluation metrics, where higher is better for PSNR and SSIM, and lower is better for LPIPS. Unless otherwise specified, all methods are evaluated under the same data split, and non-training frames in each sequence are used for sequence-internal novel-view evaluation.
...
Table~\ref{tab:r3live_rendering} reports the quantitative results on the R3LIVE (Avia) dataset~\cite{lin2022r} and the MCD (OS1-64) dataset~\cite{nguyen2024mcd}. For the R3LIVE (Avia) subset, our method achieves the best rendering metrics on all six test sequences. Compared with the closest multisensor baseline Gaussian-LIC2~\cite{lang2025gaussianlic2}, our method improves the average results on these six sequences by \textbf{1.28 dB} PSNR and \textbf{0.041} SSIM, while reducing LPIPS by \textbf{0.030} on average. In the more challenging \texttt{hku\_campus\_01} sequence, the PSNR gain reaches \textbf{2.63 dB}, indicating that the proposed context-aware feedforward insertion strategy can provide more reliable initialization for new Gaussians in difficult viewpoints and degraded regions. For the MCD (OS1-64) subset, the average PSNR and SSIM over the three sequences improve further by approximately \textbf{0.29 dB} and \textbf{0.007}, respectively, compared with Gaussian-LIC2~\cite{lang2025gaussianlic2}, while LPIPS also decreases slightly, showing that the method generalizes stably across different sensor configurations and campus scenes.
...
\begin{table*}[!t]
\centering
\caption{Comparison of sequence-wise novel-view RGB rendering results on the public R3LIVE (Avia) and MCD (OS1-64) datasets. The evaluation format is PSNR$\uparrow$ SSIM$\uparrow$ LPIPS$\downarrow$.}
...
\toprule
\textbf{Sequence} & \multicolumn{5}{c}{\textbf{Rendering Performance (PSNR$\uparrow$ SSIM$\uparrow$ LPIPS$\downarrow$)}} \\
\cmidrule(lr){2-6}
& \textbf{MonoGS} & \textbf{Co-SLAM} & \textbf{MM3DGS-SLAM} & \textbf{Gaussian-LIC2} & \textbf{Ours} \\
\midrule
\rowcolor{black!8}
\multicolumn{6}{l}{\textbf{R3LIVE (Avia)}} \\
degenerate\_seq\_00 & 14.59 \textbar{} 0.460 \textbar{} 0.624 & 14.42 \textbar{} 0.286 \textbar{} 0.810 & 19.35 \textbar{} 0.698 \textbar{} 0.212 & 21.99 \textbar{} 0.782 \textbar{} 0.162 & \textbf{23.41} \textbar{} \textbf{0.821} \textbar{} \textbf{0.137} \\
degenerate\_seq\_01 & 14.66 \textbar{} 0.438 \textbar{} 0.692 & 14.88 \textbar{} 0.267 \textbar{} 0.822 & 18.57 \textbar{} 0.621 \textbar{} 0.277 & 22.94 \textbar{} 0.790 \textbar{} 0.159 & \textbf{23.54} \textbar{} \textbf{0.810} \textbar{} \textbf{0.157} \\
hku\_campus\_00 & 15.41 \textbar{} 0.464 \textbar{} 0.675 & 15.50 \textbar{} 0.302 \textbar{} 0.794 & 18.70 \textbar{} 0.621 \textbar{} 0.319 & 24.34 \textbar{} 0.787 \textbar{} 0.156 & \textbf{25.52} \textbar{} \textbf{0.820} \textbar{} \textbf{0.152} \\
hku\_campus\_01 & 8.39 \textbar{} 0.065 \textbar{} 0.873 & 12.92 \textbar{} 0.294 \textbar{} 0.796 & 16.97 \textbar{} 0.593 \textbar{} 0.271 & 20.52 \textbar{} 0.674 \textbar{} 0.236 & \textbf{23.15} \textbar{} \textbf{0.754} \textbar{} \textbf{0.174} \\
hku\_park\_00 & 13.24 \textbar{} 0.319 \textbar{} 0.733 & 14.02 \textbar{} 0.218 \textbar{} 0.790 & 14.98 \textbar{} 0.363 \textbar{} 0.396 & 17.17 \textbar{} 0.469 \textbar{} 0.340 & \textbf{17.80} \textbar{} \textbf{0.491} \textbar{} \textbf{0.331} \\
hku\_park\_01 & 12.22 \textbar{} 0.289 \textbar{} 0.790 & 13.65 \textbar{} 0.207 \textbar{} 0.815 & 15.79 \textbar{} 0.392 \textbar{} 0.409 & 19.46 \textbar{} 0.529 \textbar{} 0.325 & \textbf{20.65} \textbar{} \textbf{0.580} \textbar{} \textbf{0.248} \\
\rowcolor{black!8}
\multicolumn{6}{l}{\textbf{MCD (OS1-64)}} \\
tuhh\_day\_02 & 8.72 \textbar{} 0.158 \textbar{} 0.893 & 12.65 \textbar{} 0.295 \textbar{} 0.766 & 11.15 \textbar{} 0.478 \textbar{} 0.350 & 20.35 \textbar{} 0.626 \textbar{} 0.262 & \textbf{20.99} \textbar{} \textbf{0.652} \textbar{} \textbf{0.249} \\
tuhh\_day\_03 & 8.09 \textbar{} 0.136 \textbar{} 0.896 & 12.48 \textbar{} 0.407 \textbar{} 0.684 & 11.36 \textbar{} 0.542 \textbar{} 0.301 & 21.38 \textbar{} 0.672 \textbar{} \textbf{0.229} & \textbf{21.82} \textbar{} \textbf{0.688} \textbar{} 0.238 \\
tuhh\_day\_04 & 11.73 \textbar{} 0.250 \textbar{} 0.805 & 13.02 \textbar{} 0.413 \textbar{} 0.626 & 13.86 \textbar{} 0.384 \textbar{} 0.398 & \textbf{19.27} \textbar{} \textbf{0.528} \textbar{} 0.310 & 19.07 \textbar{} 0.506 \textbar{} \textbf{0.309} \\
\bottomrule
\end{tabular}%
}
\end{table*}
...
\begin{tabular}{cccc}
\small GT & \small MonoGS & \small Gaussian-LIC2 & \small Ours \\
...
\end{tabular}
\caption{Qualitative comparison results on the R3LIVE dataset. The first row shows frame 690 of the \texttt{hku\_campus\_00} sequence, and the second row shows frame 516 of the \texttt{degenerate\_seq\_00} sequence. Compared with MonoGS and Gaussian-LIC2, our method preserves clearer structure around ground textures, pillar boundaries, and bright regions while significantly reducing blur, drag, and local collapse.}
\label{fig:qualitative_compare}
\end{figure*}

Figure~\ref{fig:qualitative_compare} presents two representative qualitative results. For the indoor-outdoor transition scene in \texttt{hku\_campus\_00}, MonoGS fails to recover a stable structure almost entirely, while Gaussian-LIC2 reconstructs the main geometric outline but still shows obvious blur and edge softness near the strongly exposed exit and the right-side pillar region. In contrast, our method better preserves the floor-tile texture and pillar contour. In the \texttt{degenerate\_seq\_00} scene, MonoGS exhibits large-scale collapse and incorrect color accumulation, while Gaussian-LIC2 recovers the main structure but still shows strong drag in the bright region at the lower left. Our method is further superior in both overall clarity and local consistency.
...
\begin{table}[!t]
\centering
\caption{Ablation results of the feedforward regressor (\texttt{hku\_campus\_00}). Statistics are based on the optimization trajectories of all training frames: the first two rows show mean $\pm$ std at 16 optimization rounds; the middle two rows show AUC over the first 16 rounds; and the last two rows show the mean $\pm$ std number of optimization rounds required to reach the specified quality thresholds, with the number of frames that reached the threshold shown in parentheses.}
