...
本系统是一个基于外部位姿前端的增量式 2DGS 场景重建后端。系统接收外部 LiDAR-Inertial-Camera 前端（如 R3Live）实时输出的同步数据流，每帧包含 RGB 图像、稀疏 LiDAR 点云和 6-DOF 相机位姿，以 2DGS surfel 为基础地图表示。整体流程如图 ~\ref{fig:system_pipeline} 所示。

当新关键帧到达时，系统首先优化该相机位姿，即通过将已有高斯渲染至当前帧图像上，通过光度误差优化当前帧位姿。然后，系统插入新高斯并优化已有高斯：系统将稀疏 LiDAR 点投影到图像平面形成稀疏深度图，经 SPNet 深度补全后在 LiDAR 盲区生成补充初始 3D 点；与此同时，DA3 单目深度估计模型预测稠密相对深度并与由 LiDAR 点得到的稀疏绝对深度对齐，由此为初始点提供法线先验。在初始点生成后，系统对当前高斯地图进行渲染，依据渲染透明度和颜色误差识别欠重建区域，并结合颜色梯度过滤与体素下采样，从中筛选出确需新增高斯的候选点子集。随后，系统以这些候选点作为掩码（mask）对特征图与法线图进行过滤，提取出点对齐（point-aligned）的特征，由此构造出前馈回归网络所需的两类输入：上下文特征 $\mathbf{ctx}$ 与几何先验 $\mathbf{g}$。该网络通过跨注意力机制融合多视角上下文，在单次前向传播中直接回归每个候选点的完整 surfel 属性，预测出的高质量新高斯插入地图。随后系统抽取关键帧进行在线优化，以光度损失 $\mathcal{L}_{\text{photo}}$ 和几何约束损失（$\mathcal{L}_{\text{dist}}$ 与 $\mathcal{L}_{\text{normal}}$）联合驱动高斯属性收敛。非关键帧仅接收位姿而不触发高斯插入。

前馈回归网络通过自蒸馏方式训练，无需依赖外部标注数据。其核心思想是在训练场景上利用系统自身在线优化后的高斯结果构造监督信号，使网络能够学习更好的新 surfel 初始化策略。

\subsection{稠密几何先验}

采用 SPNet 轻量级深度补全网络将 RGB 图像与稀疏 LiDAR 深度融合为稠密深度图，并在无 LiDAR 覆盖的区域采样补充 3D 初始点。最终，SPNet 补充点与原始 LiDAR 投影点合并构成初始点集。

相较于直接由 SPNet 补全深度求导获得法线，DA3 具有更强的场景结构先验，能够提供更平滑、稳定的表面方向估计。因此，本文使用 DA3 对当前帧 RGB 图像推理输出相对深度图，并通过最小二乘仿射对齐将其与稀疏 LiDAR 深度配准，得到与真实尺度一致的稠密深度图；随后以中心差分计算逐像素法线。按初始点的像素坐标在法线图上采样，得到每个初始点的相机空间法线 $\mathbf{n}_{\text{cam}} \in \mathbb{R}^3$。
...
\begin{figure*}[!t]
    ...
    \caption{上下文感知前馈高斯回归器结构。网络首先在滑动窗口内对有效候选点构造逐点对齐的上下文，并通过冻结的共享主干提取当前帧与历史帧的图像-渲染特征；随后，跨注意力模块汇聚多视图外观信息，并与逆深度、DA3 法线和位置编码等几何先验一起输入残差 MLP 编码器；最后，多头约束解码器预测 2DGS surfel 属性 $(s, \alpha, c, q)$，其中法线引导的旋转分支通过 Gram-Schmidt 正交化构造几何一致的朝向。}
    \label{fig:net-pipeline}
\end{figure*}

如图~\ref{fig:net-pipeline} 所示，本文的前馈高斯回归器以候选点为中心组织输入与预测流程。具体而言，系统首先利用当前地图的 $\alpha$-渲染结果定位欠重建区域，并据此对深度补全结果进行筛选，得到当前关键帧中的候选点；随后，围绕每个候选点聚合多视图上下文特征，并结合其几何先验，经共享编码器与多头解码器输出完整 surfel 属性。与 MVSplat 一类直接在图像网格上预测高斯参数的方法不同，本文考虑的场景中，候选点已由上游几何模块预先确定。因此，本文的目标不是学习从多视图图像到像素对齐（pixel-aligned）surfel 的稠密映射，而是以当前关键帧筛选得到的 $N_t$ 个候选点为预测单元，在多视图特征图上为每个候选点采样并聚合其对应上下文，进而学习从点对齐（point-aligned）的上下文特征 $\mathbf{ctx}$ 与几何先验 $\mathbf{g}$ 到 surfel 参数的映射。记 $\mathbf{ctx}_i$ 为候选点 $i$ 的上下文特征输入，$\mathbf{g}_i$ 为其几何先验输入，则前馈网络的目标可写为：
...
\methodlead{1) 上下文特征输入 $\mathbf{ctx}$：}共享 backbone（沿用 pixelSplat 设计，训练时冻结）分别对当前帧原始图像、当前帧渲染图像以及滑窗内 $W$ 帧历史关键帧的原始与渲染图像提取特征图。其中，由于单视角观测对 surfel 属性回归的约束有限，本文引入滑动窗口内的历史关键帧以提供多视图上下文；渲染图像使回归器能显式感知当前地图的重建状态，其与原始图像的差异隐式反映欠重建程度。对每个候选点，系统将其投影到当前帧与各历史关键帧，在对应像素位置对上述特征图进行双线性插值采样，并计算归一化观察方向；随后，将原始图像特征、渲染图像特征与编码后的观察方向向量拼接，得到当前帧逐点特征 $\mathbf{f}_i^{\text{curr}} \in \mathbb{R}^{D}$。类似地，对滑窗内 $W$ 帧可见视角执行相同操作，得到历史帧特征 $\mathbf{f}_i^{\text{hist}} \in \mathbb{R}^{W \times D}$，其中不可见的历史视角由可见性掩码 $\mathbf{m}_i^{\text{hist}} \in \{0,1\}^{W}$ 标记。随后，系统使用跨注意力模块将可变长度的历史观测压缩为固定维度的时序上下文，其中不可见视角在 softmax 前以 $-\infty$ 掩码排除：

\begin{equation}
\mathbf{ctx}_i^{\text{attn}} = \text{softmax}\!\left(\frac{\mathbf{q}_i \mathbf{K}_i^\top}{\sqrt{d_k}} + \mathbf{M}_i\right) \mathbf{V}_i
\end{equation}

\noindent 聚合完成后，当前帧特征$\mathbf{f}_i^{\text{curr}}$ 与上下文特征$\mathbf{ctx}_i^{\text{attn}}$ 拼接，构成送入编码器的上下文特征 $\hat{\mathbf{ctx}}_i$。
