...
\methodlead{1) 上下文特征输入 $\mathbf{ctx}$：}回归器的第一部分输入是逐点对齐的上下文特征输入 $\mathbf{ctx}_i$。这里的 $\mathbf{ctx}_i$ 特指注意力聚合前的上下文观测集合。与 pixelSplat、MVSplat 等需要同时从图像中恢复几何与高斯属性的离线纯视觉方法不同，本文的候选点位置已经由 LiDAR 与深度先验确定，因此网络不再负责隐式恢复位置，而是围绕每个候选点构造与之严格对齐的多视图上下文。
...
可见，仅当候选点在对应历史视角下可见时，该视角的键值对才参与注意力计算；不可见视角的注意力得分在 softmax 前被置为 $-\infty$，从而完全排除其贡献。这样，网络能够在多视图观测不完整的情况下自适应地关注真正提供补充信息的历史视角。注意力聚合完成后，系统将当前帧原始特征、当前帧渲染特征和聚合后的历史上下文拼接，得到送入编码器的上下文特征
\begin{equation}
\hat{\mathbf{ctx}}_i = [\mathbf{f}_i^{\text{curr}} \| \mathbf{f}_i^{\text{ren}} \| \mathbf{ctx}_i^{\text{attn}}].
\end{equation}
...
\methodlead{3) 回归器编码器：}在获得送入编码器的上下文特征 $\hat{\mathbf{ctx}}$ 与几何先验输入 $\mathbf{g}$ 后，系统将二者拼接为最终网络输入：
\begin{equation}
\mathbf{x}_i = [\hat{\mathbf{ctx}}_i \| \tilde{\mathbf{g}}_i].
\end{equation}
