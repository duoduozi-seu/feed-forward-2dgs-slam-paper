...
前馈回归网络的训练通过自蒸馏训练完成。系统首先以 \texttt{naive} 模式（不使用前馈网络，采用传统的基于规则初始化）在训练场景上运行一遍完整的增量式建图；当某帧新增高斯的累积训练次数达到预设阈值后，系统快照其当前属性，并在该时刻构造对应的逐点上下文与几何先验，形成训练样本，随后离线训练前馈网络。
...
\methodlead{2) 跨注意力时序上下文融合：}为使回归器具备对添加的新高斯的多视图信息，系统使用跨注意力模块将 $W$ 帧历史上下文聚合为固定维度的历史上下文向量。查询向量 $\mathbf{q}_i$ 由当前帧的观察方向、图像特征和渲染特征拼接后经线性投影得到；键和值向量 $\mathbf{k}_{i,k}$、$\mathbf{v}_{i,k}$ 则由对应历史帧的同类信息构造。时序上下文通过带可见性掩码的缩放点积注意力计算：

\begin{equation}
\mathbf{ctx}_i^{\text{attn}} = \text{softmax}\!\left(\frac{\mathbf{q}_i \mathbf{K}_i^\top}{\sqrt{d_k}} + \mathbf{M}_i\right) \mathbf{V}_i
\end{equation}

其中 $d_k = 64$，若某点的投影点位于视锥外，则$\mathbf{M}_i$ 将对应位置填充为 $-\infty$ 以完全排除其贡献。通过这种方式，系统能处理xx情况（TODO），并具备更大的时空感受野，从而能对新添加的高斯属性做出更好预测
