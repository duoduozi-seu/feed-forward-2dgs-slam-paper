...
与 MVSplat 一类直接从多视图图像恢复高斯参数的方法不同，本文关注的是在候选点已经由上游几何模块确定的条件下，学习从上下文特征输入 $\mathbf{ctx}$ 与几何先验输入 $\mathbf{g}$ 到 surfel 参数的映射。因此，instead of 回归 pixel-aligned 的 surfel，本文根据当前关键帧筛选得到的 $N_t$ 个候选点构造输入，回归 point-aligned 的 surfel。记 $\mathbf{ctx}_i$ 为点 $i$ 的上下文特征输入，$\mathbf{g}_i$ 为其几何先验输入，则前馈网络的目标可写为：
...
\methodlead{1) 上下文特征输入 $\mathbf{ctx}$：}共享 backbone（沿用 pixelSplat 设计，训练时冻结）分别对当前帧原始图像、当前帧渲染图像以及滑窗内 $W$ 帧历史关键帧的原始与渲染图像提取特征图。其中渲染图像使网络能显式感知当前地图的重建状态，其与原始图像的差异隐式反映欠重建程度。对每个候选点，系统以其像素坐标在上述特征图上双线性插值采样，并计算归一化观察方向，将对应像素坐标上的原始图像特征、渲染图像特征、编码后的观察方向向量拼接后得到当前帧逐点特征 $\mathbf{f}_i^{\text{curr}} \in \mathbb{R}^{D}$ ；类似的，对滑窗内 $W$ 帧做同样处理，得到历史帧特征 $\mathbf{f}_i^{\text{hist}} \in \mathbb{R}^{W \times D}$，其中不可见的历史视角由可见性掩码 $\mathbf{m}_i^{\text{hist}} \in \{0,1\}^{W}$ 标记。随后，系统使用跨注意力模块将可变长度的历史观测压缩为固定维度的时序上下文，其中不可见视角在 softmax 前以 $-\infty$ 掩码排除：
