...
1. **上下文感知前馈高斯回归网络。** 我们提出了一种面向增量式 2DGS 场景 SLAM 的前馈网络，用学习式预测替代传统手工初始化。该网络针对候选点云，联合利用当前帧与历史帧的原始图像特征和渲染图像特征，并通过逐点跨注意力融合多视角观测方向与可见性信息，在单次前向传播中直接回归完整的 2DGS surfel 属性。

2. **面向 2DGS surfel 的几何约束回归参数化。** 我们设计了一套将 2DGS 表面表示与 DA3 法线先验深度耦合的回归参数化机制。鉴于 2DGS surfel 具有明确的局部平面几何意义，系统首先利用 SPNet 对稀疏 LiDAR 深度进行补全以确定候选点位置，随后以 DA3 单目深度估计提供的法线先验为初始方向，由前馈网络预测该法线的修正量及辅助向量，并通过 Gram-Schmidt 正交化过程严格推导出完整的旋转矩阵。其余属性（尺度、位置、颜色）亦采用残差式或约束式参数化，确保回归结果在保持高表达能力的同时具备几何可解释性与稳定性。

3. **面向在线增量建图的自蒸馏训练闭环。** 我们构建了一套使前馈网络在无需外部标注数据集条件下完成训练的自蒸馏管线。该管线以 naive 系统在线优化并经离线 refine 后的高斯参数作为监督信号，以回溯采集的上下文特征作为输入，并通过显式清洗以消除 2DGS 尺度-旋转歧义，从而以高斯为核心单位构造训练样本，实现从在线建图、离线训练到重新部署的自增强闭环。
