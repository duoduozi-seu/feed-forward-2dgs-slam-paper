...
2. **上下文感知的前馈高斯插入方法。** 我们提出了一种用于替代传统手工初始化的前馈高斯属性回归方法。该方法对每个候选点联合利用当前帧与历史帧的原始与渲染图像特征，通过逐点跨注意力融合多视角上下文，并以法线锚定的旋转构造、2.5D 残差位置修正和密度自适应尺度等几何约束参数化，在单次前向传播中直接回归完整的 2DGS surfel 属性。
